...
- **cap060** — Upgrade system: `cutip info` shows PyPI version check, `cutip upgrade` with extensible migration registry (5 migrations), `cutip diff` for previewing changes, common command workflows in `cutip --help`
- **cap060** — `stage()` workflow separator: groups actions into named pipeline lanes for cutip-desktop DAG visualization
- **cap060** — `extract_staged_action_order()`: AST parser groups actions by stage boundaries, returns `StageGroup` objects
...
- **cap058** — Prehook ctx fix: Explicit hook declarations in unit YAML (`hooks.prehook`) now correctly receive the `CutipContext` argument. Previously, only convention-based prehook resolution (unnamed `prehook.py`) passed `ctx`, causing `AttributeError: 'NoneType' object has no attribute 'project_root'` for units with declared hooks.
...
- **cap057** — Validate orchestrator: `cutip validate` now checks `orchestrator.py` before `workflow.py`, matching `WorkflowLoader` resolution order. Groups with only an orchestrator (e.g. hello-world scaffold) no longer fail validation.
...
- **cap051** — Unit lifecycle: Per-unit `prehook.py` and `posthook.py` replace legacy `startup.py` hooks, with backward-compatible fallback and deprecation warnings
- **cap052** — Group orchestrator: `orchestrator.py` sequences unit workflows via `ctx.run_unit(name)`, with automatic fallback to `workflow.py` or declaration-order execution
- **cap053** — Safe deletion: `cutip rm group/unit/card` archives artifacts to `.cutip/trash/` before removal, with referential integrity checks
- **cap054** — Export/import: `cutip export <group>` packages groups into portable `.cutip.tar.gz` archives, `cutip import` extracts with path traversal protection
- **cap055** — Hello-world scaffold: `cutip init` now creates a hello-world project demonstrating the full lifecycle (prehook → workflow → posthook → orchestrator), `--blank` flag for minimal structure
- **cap056** — Project config: Optional `config.yaml` loaded into `ctx.config` dict, referenced from `project.config` in `cutip.yaml`
...
- **cap048** — CLI commands: Renamed `cutip plan` to `cutip preview` (hidden alias retained), enhanced `cutip info` with artifact counts and per-unit hook status table, added `cutip create` subcommands for scaffolding cards, units, and groups
- **cap049** — Completion cache: Shell tab completion now reads group names from `.cutip/cache/groups.json` instead of running full workspace discovery, with automatic cache refresh on every `cutip run`
- **cap050** — Annotations + compile: New decorators (`@prehook`, `@posthook`, `@healthcheck`, `@cleanup`, `@config`) for self-describing workflow steps, plus `cutip compile <group>` to generate Mermaid flowcharts and summary tables
...
- **cap045** — Dark theme: MkDocs documentation now uses a dark theme by default with custom CSS design tokens reusable by cutip-desktop
- **cap046** — README trim: README trimmed to essential content -- badges, identity, comparison table, install, getting started, and docs link
- **cap047** — Documentation rewrite: New concept pages (lifecycle, annotations), new reference pages (compile, themes), and updated existing docs to reflect CUTIP identity
...
- **cap043** — Semantic versioning convention: Documented semver policy (x=major, y=minor, z=patch) in CLAUDE.md for consistent release versioning
- **cap044** — ruff + ty tooling: Added ruff linter/formatter and ty type checker as CI merge gate checks, reformatted entire codebase, fixed all lint violations
